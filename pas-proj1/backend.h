 ...
 *   backend.h 
 *
 *   definitions and function prototypes for backend.c
 *
 */
...
/* b_global_decl emits the pseudo-op .data if beginning an data
   ...
   b_global_decl("i", sizeof(int));
   b_alloc_int(5);

   For another example, to emit code for the global declaration
   ``int a[10] = {3,4,5};'', one might call

   b_global_decl("a", sizeof(int));
   b_alloc_int(3);
   b_alloc_int(4);
   b_alloc_int(5);
   b_skip(7*sizeof(int));
   */
void b_global_decl (char *id, int alignment);
...
/* This is not really a back end function.  It is defined in the utils.c
   file in the Project 1 solution.  It provides a uniform interface to the
   seven functions above.
*/
...
/* b_get_local_var_offset returns the current value of loc_var_offset.
   ...
   passive--it emits no assembly code and has no effect on state variables.
*/
...
/* b_alloc_arglist takes the total size (in bytes) of actual arguments in a
   ...
   actual_arg_word_count[aawc_top] to 0.  This value is used to count the
...
/* b_funcall_by_name accepts a function name and a
   ...
   actual_arg_word_count[aawc_top] to find the amount of space taken
...
/* b_funcall_by_ptr accepts the return type for a function, and when
   ...
   the function onto the stack.  Uses actual_no[an_top] to find
